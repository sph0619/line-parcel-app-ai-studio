...
import { CheckCircle2, Image as ImageIcon, UserCircle } from 'lucide-react';
          ...
          <tbody className="divide-y divide-slate-100">
            {historyItems.map((pkg) => (
              <tr key={pkg.packageId} className="hover:bg-slate-50">
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="font-black text-blue-700 text-base">{pkg.householdId}</span>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-3">
                     <div className="hidden md:flex items-center gap-1 text-emerald-600 bg-emerald-50 px-2 py-1 rounded text-[10px] font-bold border border-emerald-100">
                        <CheckCircle2 size={12} />
                        驗證通過
                     </div>
                     {pkg.signatureDataURL && pkg.signatureDataURL !== 'Manual Pickup' ? (
                       <div className="group relative">
                          ...
                          <div className="absolute bottom-full right-0 mb-3 hidden group-hover:block z-50 bg-white p-3 shadow-2xl rounded-2xl border border-slate-200 min-w-[200px]">
                            ...
                            <p className="text-[9px] text-center text-slate-300 mt-2 italic">此圖檔為法律存查之依據</p>
                          </div>
                       </div>
                     ) : (
                        <span className="text-[10px] text-amber-500 font-bold bg-amber-50 px-2 py-1 rounded border border-amber-100">手動</span>
                     )}
                  </div>
                </td>
              </tr>
